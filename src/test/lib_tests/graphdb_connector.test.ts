...
describe("DbConnector", () => {
  ...
  it("create a Book node", (done) => {
    dbConnector.createBookNode(bookData, (err, bookNode: Node) => {
      ...
      if (!bookNode) {
        return done(
          new Error("No book node provided in createBookNode() callback")
        );
      }
      try {
  ...
  it("inserting a Book node as a child of another Book node that does not exist creates the child with no relationship", (done) => {
    dbConnector.createChildBookNodeAndRelations(
      "rubbish_ASIN",
      childBookData,
      (err) => {
        if (err) {
        ...
        test_utils.checkOnlyOneNodeMatchesAsin(
          dbConnector,
          { asin: childBookData.ASIN },
          (err) => {
            // new "child" node exists
            if (err) {
              return done(err);
            }
            dbConnector.run(
              ...
              (err, result) => {
                ...
                if (!result)
                  return done(
                    new Error("Expected result to be present (truthy)")
                  );
