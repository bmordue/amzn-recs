...
function handleSuccess(responseJson, req, res) {
  ...
  log.info(
    { status: res.statusCode, method: req.method, url: req.url },
  ...
  whitelist.check(token, (err, whitelisted) => {
    if (err) {
      return handleServerError(
        503,
        "Failed to check token whitelisting",
        req,
        res
      );
    }
    if (!whitelisted) {
      return handleClientError(
        403,
        `Token has not been whitelisted: ${token}`,
        req,
        res
      );
    }
...
router.put("tasks", (req, res) => {
  ...
  switch (taskStatus) {
    ...
    case MessageQueue.STATUS_WAITING:
      ...
      break;
    default:
      updateTaskFn = function (_, cb) {
        cb(new Error("Unrecognised status"));
      };
      handleBadRequest(
        util.format("Unrecognised status: %s", taskStatus),
        req,
        res
      );
  }
