...
function paths() {
  ...
  session.run('Match p=((n) -[*11]-> (m)) return p limit 3;')
    .subscribe({
      onError: () => { session.close(); process.exit(1); },
      onNext: (record) => { record.get('p').segments.forEach((el, i) => { console.log(el.start); }); },
      onCompleted: () => { session.close(); process.exit(); },
    });
}

function main() {
  simpleQuery('MERGE (b:Book { ASIN: "B019CSNQ24" }) SET b.Title = "The Nightmare Stacks: A Laundry Files novel", b.Author = "Charles Stross" RETURN b', () => {
