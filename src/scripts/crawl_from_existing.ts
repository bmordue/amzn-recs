...
const checkAndCallback = function (errors, callback) {
  if (errors.length) {
    log.error({ errors }, util.format('%s errors while crawling', errors.length));
...
const crawl_from_existing = function (callback) {
  ...
  crawler.db.listLeafNodeAsins((err, leaf_nodes) => {
    if (err) {
      return callback(err);
    }
    const crawl_errors = [];
