...
CrawlQueue.prototype.crawl = function(rootAsin, depth, callback) {
	...
	this.nodeCount++;
	log.debug({}, util.format("Current crawl depth: %s, parent node ASIN ", depth, rootAsin));
	var self = this;
	depth += 1;
	...
	this.throttledSimilarityLookup(rootAsin, function(err, result) {
		...
		fs.writeFile(path.join("output", rootAsin + ".json"), JSON.stringify(result), function(err) {
			if (err) {
				log.warn(err, "Error writing lookup result to file");
			}
		});
		// log.debug(result, 'Similarity lookup result for ASIN ' + rootAsin);
		async.each(result.Items.Item, function(item, each_cb) {
			self.alreadyCrawled(item.ASIN, function(err, result) {
				...
				if (!result.crawled) {
					self.db.createChildBookNode(rootAsin, item, function(err, node) {
						if (err) {
							return each_cb(err);
						}
					...
					// log.debug(result, util.format("Already crawled %s", item.ASIN));
					log.debug({}, util.format("Already crawled %s", item.ASIN));
					each_cb();
				}
			});
		}, callback);
