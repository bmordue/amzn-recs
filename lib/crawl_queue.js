...
function CrawlQueue(options) {
	...
	fs.mkdirSync(CrawlQueue.inputDir);
}
...
CrawlQueue.prototype.crawl = function(rootAsin, depth, callback) {
	...
	this.throttledSimilarityLookup(rootAsin, function(err, result) {
		...
		var filename = path.join(CrawlQueue.inputDir, rootAsin + ".json");
		fs.stat(filename, function(err, stats) {
			if (err && err.code == "ENOENT") {
				try {
					fs.writeFileSync(filename, JSON.stringify(result, null, 4));
				} catch (e) {
					log.error({filename: filename}, "Couldn't write file");
					return callback(e);
				}
			}
...
CrawlQueue.inputDir = process.env.INPUT_DIR || "input";
CrawlQueue.errorDir = process.env.ERROR_DIR || "input/error";
CrawlQueue.doneDir = process.env.DONE_DIR || "input/done";
