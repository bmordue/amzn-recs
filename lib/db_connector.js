...
function createChildBookNode(db, data, callback) {
	...
	db.cypher({
		...
		log.debug({err: err, result: result}, 'initial merge query complete');
		newNodeResult = result;
		if (err) {
			log.error(err, "Error in createChildBookNodeAndRelations, merge child node");
			...
			db.cypher(
				...
				function(error, result) {
					if (error) {
						log.error(error, "Error in createChildBookNodeAndRelations, match child node causing constraint violation");
