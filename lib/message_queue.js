...
MessageQueue.prototype.claim = function(callback) {
	...
	var selectStmt = db.prepare(selectQStr);
	selectStmt.get(function(err, result) {
		...
		selectStmt.finalize(function(err) {
			...
			update(db, row.id, MessageQueue.STATUS_PROCESSING, callback);
		});
	});
}

function update(db, rowid, status, callback) {
	var selectQStr = util.format("UPDATE OR FAIL %s SET status = %s WHERE rowid = %s LIMIT 1",
															CRAWL_TASKS_TABLE_NAME, status, rowid);
	var selectStmt = db.prepare(selectQStr);
	selectStmt.get(function(err, result) {
...
MessageQueue.prototype.complete = function(rowid, callback) {
	update(this.db, rowid, MessageQueue.STATUS_DONE, callback);
}
...
MessageQueue.prototype.unclaim = function(rowid, callback) {
	update(this.db, rowid, MessageQueue.STATUS_WAITING, callback);
}
